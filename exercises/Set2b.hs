...
oddFactorial n = 
    if n == 1 then 1
    else if odd n then n*oddFactorial (n - 2) else oddFactorial (n - 1)
...
leftpad s l = if l > length s then leftpad (" " ++ s) l else s 
...
countdown second = "Ready! " ++ count second ++ "Liftoff!"
    where count second = if second /= 0 then show second ++ "... " ++ count (second - 1) else ""
...
isPrime n = if n == 0 || n == 1 || (smallestDivisor n) /= n  then False else True
