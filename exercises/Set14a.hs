...
greetText t = T.pack "Hello, " <> (if T.length t > 15 then T.take 15 t <> T.pack "...!" else t <> T.pack "!")
...
shout :: T.Text -> T.Text
shout t = if null $ T.words t then t else T.unwords $ zipWith (\x y -> if even x then T.toUpper y  else y ) [0..length $ T.words t] (T.words t)
...
longestRepeat t = case T.foldr (\s (x, y ,z) -> if s == z then (max x $ y + 1, y + 1, z) else (x, 1, s)) (0, 0, 'a') t of (ans, k, _) -> max ans k;
...
byteRange b = if B.null b then 0 else B.foldl max (B.head b) b - B.foldl min (B.head b) b
