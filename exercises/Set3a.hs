...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (\y -> y <= max) [k ^ i | i <- [0..114514114514]]
...
joinToLength = \x y -> filter (\c -> length c == x) [a ++ b | a <-  y, b <- y]
...
(+|+) x y = 
    m ++ n
        where 
            m = if null x then [] else take 1 x
            n = if  null y then [] else take 1 y
...
multiCompose fs = if null fs then id else (head fs) . (multiCompose $ drop 1 fs)
...
interpreter commands = 
    helper 0 0 [] 0 
        where 
            helper x y ans loc =
                if loc == length commands then reverse ans 
                else
                    case commands !! loc of
                        "up" -> helper x (y + 1) ans (loc + 1)
                        "down" -> helper x (y - 1) ans (loc + 1)
                        "left" -> helper (x - 1) y ans (loc + 1)
                        "right" -> helper (x + 1) y ans (loc + 1)
                        "printX" -> helper x y ((show x) : ans) (loc + 1)
                        "printY" -> helper x y ((show y) : ans) (loc + 1)
