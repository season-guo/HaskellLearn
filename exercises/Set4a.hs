...
import qualified GHC.Arr as Data.Array
...
allEqual xs = 
    case xs of 
        x : y : left -> if x /= y then False else allEqual (y : left)
        x : [] -> True
        [] -> True 
...
middle =   \x y z -> max (min (max x y) z ) (min (max x z) y)
...
longest :: (Ord a) => [[a]] -> [a] 
longest = \xs -> foldr (\x y -> if length x > length y then x else if length x == length y then (if (x !! 0) < (y !! 0) then x else y) else y) (head xs) xs
...
incrementKey = \k m -> map (\x -> if fst x == k then (k, (snd x) + 1 ) else x) m
...
freqs xs = 
    foldr (\x now -> Map.alter f x now) Map.empty xs  
        where 
            f x =
                case x of 
                    Nothing -> Just 1
        ...
        case Map.lookup from bank of
            ...
            Just a -> 
                case Map.lookup to bank of
                    Nothing -> bank
                    Just b -> if a - amount < 0 then bank else Map.insert to (amount + b) (Map.insert from (a - amount) bank)  
...
maxIndex arr = 
        foldr  (\x y -> if arr ! x > arr ! y then x else y)  (head arrlist) arrlist 
            where arrlist = Data.Array.indices arr
