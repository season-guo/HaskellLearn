...
import GHC.Arr (listArray, accumArray)
...
prettyPrint n queen =  go (1, 1)
    where 
        arr =  accumArray (\x y -> y) '.' ((1, 1), (n, n)) [(a, 'Q') | a <- queen ]
        go (row, col) = if row == n + 1 then [] else if col == n + 1 then '\n' : (go $ nextRow (row, col)) else (arr ! (row, col)) : (go $ nextCol (row, col))
...
danger cor queen = any (\x -> sameRow cor x || sameCol cor x || sameAntidiag cor x || sameDiag cor x) queen
...
prettyPrint2 n queen =  go (1, 1)
    where 
        arr =  accumArray (\x y -> y) '.' ((1, 1), (n, n)) [(a, 'Q') | a <- queen ]
        go (row, col) = if row == n + 1 then [] else if col == n + 1 then '\n' : (go $ nextRow (row, col)) else if arr ! (row, col) == 'Q' then 'Q' : (go $ nextCol (row, col)) else if danger (row, col) queen then '#' : (go $ nextCol (row, col)) else '.' : (go $ nextCol (row, col))
...
step n queen = 
    case fixFirst n queen of
        Just q -> continue q
        Nothing -> backtrack queen
...
finish n queen = if fst (head queen) <= n then finish n $ step n queen else tail queen
